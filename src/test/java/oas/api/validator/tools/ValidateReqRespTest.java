...
class ValidateReqRespTest {
	...
	@Test // issue reported by issue #314 -
	...
	@Test // issue reported #313 -
			// https://bitbucket.org/atlassian/swagger-request-validator/issues/313/validation-of-schemas-using-allof
	...
	public void testRequestExamplesMandatoryParams() {
		final String apiContent = ResourceLoader
				.getResourceAsString("openapi-specs/dog_requests_mandatory_params.yaml");
		final String validateExamples = ValidateReqResp.validateExamples(apiContent);
		assertTrue(validateExamples.contains(
				"No issue found when validating the example requests and responses for OpenAPI Specification"));
	}
